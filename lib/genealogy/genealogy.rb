  ...
  module ClassMethods
    ...
    def has_parents options = {}
      ...
      tracked_relatives.each do |k|
        belongs_to k, class_name: self.class.name, foreign_key: self.send("#{k}_id_column")
      end

      has_many :children_as_father, class_name: self.class.name, foreign_key: self.father_id_column, dependent: :nullify
      has_many :children_as_mother, class_name: self.class.name, foreign_key: self.mother_id_column, dependent: :nullify
